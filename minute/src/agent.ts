...
import { trySimpleVisualEdit, type LastVisualOp } from "./simple-edit.js";
...
function resolveChosen(
  ...
  const exact = picked.map((f) => normalizeRepoPath(f)).filter((f) => inventory.includes(f));
  if (exact.length) return exact.slice(0, LIMITS.maxEditFiles);
  const byBase = picked
    .map((f) => normalizeRepoPath(f).split("/").pop() || "")
    .filter(Boolean)
    .flatMap((base) => inventory.filter((f) => f === base || f.endsWith(`/${base}`)));
  if (byBase.length) return [...new Set(byBase)].slice(0, LIMITS.maxEditFiles);
  const lower = request.toLowerCase();
  const wantCss = /color|green|header|font|background|css|style/.test(lower);
  const fallback = ranked.find((f) => (wantCss ? f.endsWith(".css") : true) && inventory.includes(f));
...
export async function applySmallestChange(opts: {
  ...
  lastOp?: LastVisualOp;
}): Promise<{ files: string[]; summary: string; routeHint?: string; lastOp?: LastVisualOp }> {
  ...
  const simple = trySimpleVisualEdit(
    opts.root,
    opts.playground.allow.paths,
    opts.request,
    opts.lastOp,
  );
...
Paths are repo-relative with no leading slash (e.g. frontend/src/index.css).
Max ${LIMITS.maxEditFiles} files.`,
    { maxTokens: 800 },
  );

  const chosen = resolveChosen(pick.files || [], inventory, ranked, opts.request);
  ...
  }>(`You are Minute. One file, one replace when possible. Smallest change for a non-technical stakeholder.
Copy exact bytes from the file into search — include the selector or neighboring lines so it appears once.
...
Paths are repo-relative with no leading slash.`);

  let ops = parseOps(edit.ops || []);
  let written: string[] = [];
  try {
    written = ops.length
      ? applyPatchOps(opts.root, opts.playground.allow.paths, ops, opts.request)
      : [];
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    if (!/Search not found|Search not unique|Missing file/i.test(msg)) throw err;
...
Return new ops. For replace, copy a unique snippet from the file (include the selector or neighboring lines). Paths are repo-relative with no leading slash.
{
  "summary": "one line, human",
  "route": "/path",
  "ops": [{ "kind": "replace", "path": "a.css", "search": "exact unique old", "replace": "new" }]
}`);
    if (retry.summary) edit.summary = retry.summary;
    if (retry.route) edit.route = retry.route;
    ops = parseOps(retry.ops || []);
    written = ops.length
      ? applyPatchOps(opts.root, opts.playground.allow.paths, ops, opts.request)
      : [];
  }
  ...
  if (
    ...
    const asset = opts.attachments.find((a) => a.role === "asset") ?? opts.attachments[0];
    if (asset?.localPath) {
      const rel = normalizeRepoPath(pick.placeAttachmentAs);
      const dest = join(opts.root, rel);
