...
export function liveRunInThread(surface: Surface, threadId: string): Run | undefined {
  const run = runByThread(surface, threadId);
  if (!run) return undefined;
  if (run.status === "exited" || run.status === "refused" || run.status === "handed_off") return undefined;
  return run;
}
...
export function recentRuns(limit = 40): Run[] {
  const rows = getDb()
    .prepare("SELECT data FROM runs ORDER BY updated_at DESC LIMIT ?")
