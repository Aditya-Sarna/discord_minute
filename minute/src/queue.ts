...
export function startWorker(handler: Handler) {
  const tick = async () => {
    ...
    try {
      ...
      if (job.attempts < 3 && !/timeout|refused|Blocked path|isn't a Minute|BASE_TYPE|403|Permission to|Ollama is not|npm install failed|Preview never|No files changed|Executable doesn't exist/i.test(message)) {
        getDb()
