...
export function enqueueUndo(runId: string) {
  return enqueue("iterate", runId, { text: "undo that" });
}
...
export function startJobs(lookup: AdapterLookup) {
  startWorker(async (job: Job) => {
    ...
    try {
      ...
      if (job.attempts >= 3 || /timeout/i.test(message)) {
        try {
          await chat.postRefuse(`Couldn’t finish this Minute: ${stakeholderError(message)}. Ping tech if you still need it.`);
        } catch {
